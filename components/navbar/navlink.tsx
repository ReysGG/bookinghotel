        ...
        {session?.user ? (
            <div className="flex justify-end items-center md:order-2 ">
                <div className="hidden text-sm bg-gray-50 border rounded-full md:me-0 md:block focus:ring-4 focus:ring-gray-300">
                    <Image src={session.user.image || '/Frontendimage/avatar.svg'} width={64} height={64} alt="avatar" className="size-8 rounded-full"></Image>
                </div>
...
"
                >
                    ...
                    {!session ? (<>
                        ...
                        <li className="pt-4 md:pt-0">
                            <button onClick={() => signOut} className={`md:hidden py-3 px-10 bg-red-400 text-white hover:bg-red-600 rounded-sm cursor-pointer `}>Log Out</button>
                        </li>
                    )}
