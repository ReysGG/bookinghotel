...
const ContactForm = () => {
    ...
    //state responses dari server action
    // FormAction digunakan di form action
    
    return <>
        <div className="bg-white p-8 rounded-sm shadow-sm">
            {state?.message && (<>
                <div className="p-4 mb-4 text-sm text-gray-800 rounded-lg" role="alert">
                    <div className="font-medium">{state.message}</div>
                </div>
            </>)}
            <form action={FormAction}>
                <div className="grid md:grid-cols-2 gap-7 mt-6 bg-green-50">
                    <div>
                    ...
                    <div>
                        <input type="email" name="email" className="bg-gray-50 p-3 border border-gray-200 rounded-sm w-full font-light" placeholder="Johndue@exmaple.com*" />
                        <div aria-live="polite" aria-atomic="true">
                ...
                <button className={clsx("px-10 text-center py-4 font-semibold text-white w-full bg-blue-400 rounded-sm hover:bg-blue-500 cursor-pointer", {
